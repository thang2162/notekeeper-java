...
public class newNote {
    ...
    public Response consumeJSON(@HeaderParam("authorization") String auth, String Json) {
        ...
        return NewNote(auth, jsonObject.getString("title"), jsonObject.getString("note"));
    }
    ...
    @FormParam("title") String title, @FormParam("note") String note) throws IOException {
    	
    	System.out.println("Starting FORM Endpoint!");
    	
    	return NewNote(auth, title, note);
        
    }
    
    
    
    private Response NewNote (String jwt, String title, String note_txt) {
    	...
    	if(validateJwt.getIsValid()) {
    		...
    		Note note = new Note(curDate, validateJwt.getEmail(), title, note_txt);
    		
    		Boolean DaoRes = noteDao.newNote(note);
    		
    		if(DaoRes == true) {
    			String res = "{\"status\":\"success\", \"msg\":\"Note sucessfully created\"}";
